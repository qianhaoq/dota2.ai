...
- 不要绕过或削弱 CI 门禁。
- 界面保持**英文默认、中文可切换**的双语约定；DeepSeek/API 输出须跟随 UI 语言。
...
- 产品反馈：应用内「反馈 / Feedback」打开站内留言表单，提交到 `/api/feedback`（服务器写入 `data/feedback.jsonl`）；贡献与缺陷讨论仍走 GitHub Issues。
...
- Product feedback: in-app Feedback opens a leave-a-message form that POSTs to `/api/feedback` (stored as `data/feedback.jsonl` on the server); contribution/bug discussion still via GitHub Issues.
