...
export function buildMatchFact(match, opts = {}) {
  ...
  const players = (match.players || []).map((p) => {
    ...
    return {
      ...
      // Preserve absence of purchase_log — do not coerce missing → [] (that invents “common missing”).
      purchaseLog: Array.isArray(p.purchase_log)
        ? p.purchase_log.map((buy) => ({
          time: buy.time,
          key: String(buy.key || '').replace(/^item_/, ''),
        })).filter((buy) => buy.key)
        : null,
      lastHits: p.last_hits ?? (Array.isArray(p.lh_t) && p.lh_t.length ? p.lh_t[p.lh_t.length - 1] : undefined),
    };
  });
