...
function buildFocusLens(focusPlayer, allPlayers, objectives, lang, laneGrounded = true) {
  ...
  const relevantEvents = objectives.filter((o) => o.time <= 20 * 60).slice(0, 12);
...
function buildTimelineObjectives(rawObjectives, players) {
  ...
  return filtered.map((o) => {
    ...
    if (o.type === 'CHAT_MESSAGE_AEGIS') {
      const roshanAtTime = rawObjectives.find(
        (r) => r.type === 'CHAT_MESSAGE_ROSHAN_KILL' && r.time === o.time
      );
      const roshanTeam = roshanAtTime?.team ?? null;
