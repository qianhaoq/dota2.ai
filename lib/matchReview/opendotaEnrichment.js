...
export function dotabuffHeroGuidesUrl(internalSlug) {
  ...
  const slug = internalSlug.trim().replace(/^npc_dota_hero_/, '').replace(/_/g, '-');
  if (!slug) return null;
...
export function percentileForValue(rows, actual) {
  ...
  for (let i = 1; i < sorted.length; i += 1) {
    ...
    if (v >= lo.value && v <= hi.value) {
      ...
      return Math.round(p * 1000) / 10; // percent 0-100, 1 decimal
    }
  }
  return null;
}
...
/**
 * @param {{ actual: { gpm?: number, xpm?: number, lastHits?: number }, benchmarks?: object }} input
 */
export function buildBenchmarkComparison({ actual, benchmarks }) {
  ...
  if (actual?.lastHits != null) {
    const percentile = percentileForValue(result.last_hits, actual.lastHits);
    if (percentile != null) out.lastHits = { actual: actual.lastHits, percentile };
  }
  return out;
}
...
/**
 * @param {{ purchaseLog?: Array<{time:number,key:string}>, itemPopularity?: object, durationSeconds?: number }} input
 */
export function compareItemBuild({ purchaseLog = [], itemPopularity = {}, durationSeconds = 0 }) {
  const actualCore = (purchaseLog || [])
    .filter((p) => isMajorPurchase(p.key))
    .map((p) => ({
      key: p.key,
      name: p.key.replace(/_/g, ' '),
      time: p.time,
      timeLabel: formatItemTime(p.time),
    }));

  const popularMid = (itemPopularity.midGame || []).slice(0, 6);
  const popularLate = (itemPopularity.lateGame || []).slice(0, 6);
  const popularKeys = new Set(
    [...popularMid, ...popularLate, ...(itemPopularity.earlyGame || [])]
      .map((i) => i.key)
      .filter(Boolean),
  );

  const actualKeys = new Set(actualCore.map((i) => i.key));
  const midThreshold = Math.min(25 * 60, Math.max(12 * 60, (durationSeconds || 0) * 0.45));

  const delayed = popularMid
    .filter((pop) => {
      const bought = actualCore.find((a) => a.key === pop.key);
      return bought && bought.time > midThreshold + 5 * 60;
    })
    .map((pop) => {
      const bought = actualCore.find((a) => a.key === pop.key);
      return {
        key: pop.key,
        name: pop.name || pop.key,
        expectedBy: formatItemTime(midThreshold),
        actualAt: bought?.timeLabel,
      };
    });
...
/**
 * @param {{ focusHeroId: number, enemyHeroIds: number[], matchupsMap: object, heroNames?: object, lang?: string, minGames?: number }} input
 */
export function buildMatchupContext({
  ...
  for (const enemyId of enemyHeroIds) {
    ...
    const advantage = parseFloat(m.advantage);
    const winRate = m.winRate != null ? String(m.winRate) : null;
    rows.push({
      heroId: enemyId,
      heroName,
      advantage: Number.isFinite(advantage) ? advantage : 0,
      advantageLabel: Number.isFinite(advantage)
...
export function buildHeroEnrichmentPayload({
  ...
  const benchmarkComparison = buildBenchmarkComparison({
    actual: { gpm: focus.gpm, xpm: focus.xpm, lastHits },
    benchmarks,
  });

  const itemCompare = compareItemBuild({
    purchaseLog: focus.purchaseLog || [],
    itemPopularity: itemPopularity || {},
    durationSeconds: matchFact.summary?.duration || 0,
  });
...
export function buildEnrichmentSectionCards(matchFact, lang = 'zh') {
  ...
  if (enr.itemCompare) {
    ...
    for (const d of (ic.delayed || []).slice(0, 2)) {
      rows.push({
        label: isZh ? '偏晚' : 'Delayed',
        value: isZh
          ? `${d.name} 于 ${d.actualAt}（参考 ${d.expectedBy} 前）`
          : `${d.name} at ${d.actualAt} (ref before ${d.expectedBy})`,
      });
    }
    ...
    if (rows.length) {
      cards.item_compare = {
        ...
        insight: isZh
          ? '对照 OpenDota 该英雄常见中后期装，仅陈述差异，非绝对最优解。'
          : 'Compared to OpenDota popular mid/late items for this hero — descriptive, not absolute truth.',
        rows,
  ...
  if (enr.benchmarks && Object.keys(enr.benchmarks).length) {
    ...
    if (rows.length) {
      ...
      cards.farm_benchmarks = {
        ...
        insight: low
          ? (isZh
            ? '相对同英雄基准，本场经济/经验/补刀百分位偏低，值得作为改进点。'
            : 'Vs hero benchmarks, farm metrics sit in a low percentile — a concrete focus area.')
          : (isZh
  ...
  if (Array.isArray(enr.matchups) && enr.matchups.length) {
    cards.matchup_context = {
      ...
      insight: isZh
        ? '基于 OpenDota 历史对位优势（样本≥50），描述本场敌方英雄相对强弱。'
        : 'OpenDota historical matchup advantage (≥50 games) vs enemies in this match.',
...
export { isMajorPurchase, formatItemTime, CONSUMABLE_OR_COMPONENT };
