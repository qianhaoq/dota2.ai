...
/**
 * True when constants mark the item as an activatable / ability-bearing standalone.
 * @param {any} item
 */
function itemHasAbilitiesOrBehavior(item) {
  if (!item || typeof item !== 'object') return false;
  if (item.behavior != null && item.behavior !== false && item.behavior !== '') return true;
  const abs = item.abilities;
  if (!abs) return false;
  if (Array.isArray(abs)) return abs.length > 0;
  if (typeof abs === 'object') return Object.keys(abs).length > 0;
  return false;
}
...
function isMajorPurchase(key, itemConstants = null) {
  ...
  if (itemConstants) {
    ...
    if (componentKeys.has(k)) {
      // Standalone usables that also appear in upgrade recipes (blink, ghost, gem, …).
      // Base boots are a major early purchase despite empty abilities.
      if (k === 'boots') return true;
      if (itemHasAbilitiesOrBehavior(item)) return true;
      return false;
    }
  }
...
export function compareItemBuild({ purchaseLog = [], itemPopularity = {}, itemConstants = null, durationSec = null }) {
  ...
  const popularKeys = new Set(
    [...popularMidFull, ...popularLateFull, ...popularEarly].map((i) => i.key),
  );
  // Comparator ignores startGame; empty early/mid/late means no usable popularity.
  if (popularKeys.size === 0) {
    return {
      unavailable: true,
      actualCore: [],
      popularMid: [],
...
export { isMajorPurchase, formatItemTime, CONSUMABLE_OR_COMPONENT, DOTABUFF_SLUG_BY_INTERNAL, buildRecipeComponentKeySet };
