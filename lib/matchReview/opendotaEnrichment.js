...
/** WeakMap cache: itemConstants object -> Set of recipe component keys */
const recipeComponentKeyCache = new WeakMap();

/**
 * Collect every item key listed as a recipe component in item constants.
 * @param {Record<string, { components?: string[] | null }> | Map | null | undefined} itemConstants
 * @returns {Set<string>}
 */
export function buildRecipeComponentKeySet(itemConstants) {
  const empty = new Set();
  if (!itemConstants) return empty;
  if (typeof itemConstants === 'object' && recipeComponentKeyCache.has(itemConstants)) {
    return recipeComponentKeyCache.get(itemConstants);
  }
  const keys = new Set();
  ...
  for (const [, item] of entries) {
    if (!item || !Array.isArray(item.components)) continue;
    for (const c of item.components) {
      if (!c || typeof c !== 'string') continue;
      const k = c.trim().toLowerCase().replace(/^item_/, '');
      if (k) keys.add(k);
    }
  }
  if (typeof itemConstants === 'object') {
    recipeComponentKeyCache.set(itemConstants, keys);
  }
  return keys;
}

/**
 * @param {Record<string, any> | Map | null | undefined} itemConstants
 * @param {string} key
 */
function lookupItemConstant(itemConstants, key) {
  if (!itemConstants || !key) return null;
  if (itemConstants instanceof Map) return itemConstants.get(key) || null;
  return itemConstants[key] || null;
}

/**
 * Standalone shop items that also appear as upgrade recipe inputs.
 * OpenDota `/constants/items` is not a reliable source for `behavior`/`abilities`,
 * so keep these as major purchases via explicit keys + crafted-item metadata.
 */
const STANDALONE_MAJOR_PURCHASES = new Set([
  'blink',
  'ghost',
  'boots',
]);
...
/**
 * Crafted / completed item (has its own recipe), even if later used as an upgrade input.
 * @param {any} item
 */
function itemIsCraftedOrCombined(item) {
  if (!item || typeof item !== 'object') return false;
  if (item.created === true) return true;
...
/**
 * @param {string} key
 * @param {Record<string, any> | Map | null} [itemConstants]
 * @returns {boolean}
 */
function isMajorPurchase(key, itemConstants = null) {
  const k = normalizeItemKey(key, itemConstants);
  if (!k) return false;
  if (k.startsWith('recipe_') || k === 'recipe') return false;

  const item = itemConstants ? lookupItemConstant(itemConstants, k) : null;
  if (item && String(item.qual || '').includes('consumable')) return false;

  // Explicit exclusions win over crafted upgrade-input promotion
  // (e.g. urn_of_shadows → Spirit Vessel must not enter actualCore).
  if (CONSUMABLE_OR_COMPONENT.has(k)) return false;

  // Derive component membership from recipe data when constants are present.
  // Catches patch-sensitive parts omitted from CONSUMABLE_OR_COMPONENT (relic, talisman_of_evasion).
  if (itemConstants) {
    const componentKeys = buildRecipeComponentKeySet(itemConstants);
    if (componentKeys.has(k)) {
      // Completed items that feed further upgrades (force_staff→pike, vanguard→crimson, …).
      if (itemIsCraftedOrCombined(item)) return true;
      // Standalone shop usables that are also upgrade inputs (blink→*blink, ghost→ethereal).
      // Do NOT rely on behavior/abilities — those fields are absent from this endpoint's
      // usable metadata surface for this check.
      if (STANDALONE_MAJOR_PURCHASES.has(k)) return true;
      return false;
    }
  }
...
/** Align with reviewCards phase cuts: mid starts ~10:00, late ~25:00. */
const POPULARITY_MID_START_SEC = 10 * 60;
const POPULARITY_LATE_START_SEC = 25 * 60;
 ...
 * @param {{ purchaseLog?: Array<{time:number,key:string}> | null, itemPopularity?: object, itemConstants?: object, durationSec?: number | null }} input
 */
export function compareItemBuild({ purchaseLog = [], itemPopularity = {}, itemConstants = null, durationSec = null }) {
  ...
  // Membership from COMPLETE fetched popularity lists; slice only for display rows.
  // Otherwise ranks 7–10 get labeled "Uncommon" despite being in the popular set.
  const popularMidFull = normalizePop(itemPopularity.midGame);
  const popularLateFull = normalizePop(itemPopularity.lateGame);
  const popularEarly = normalizePop(itemPopularity.earlyGame);
  const earlyKeys = new Set(popularEarly.map((i) => i.key));
  const midKeys = new Set(popularMidFull.map((i) => i.key));
  const lateKeys = new Set(popularLateFull.map((i) => i.key));
  // Comparator ignores startGame; empty early/mid/late means no usable popularity.
  if (earlyKeys.size + midKeys.size + lateKeys.size === 0) {
    return {
      unavailable: true,
      actualCore: [],
      popularMid: [],
  ...
  const popularMid = popularMidFull.slice(0, 6);
  const popularLate = popularLateFull.slice(0, 6);

  // Stages the match never reached must not feed "Common missing" claims.
  // Still expose popularMid/popularLate as non-missing reference rows when present.
  const duration = Number(durationSec);
  const hasDuration = Number.isFinite(duration) && duration > 0;
  const reachedMid = !hasDuration || duration >= POPULARITY_MID_START_SEC;
  const reachedLate = !hasDuration || duration >= POPULARITY_LATE_START_SEC;
  const missingCandidates = [
    ...(reachedMid ? popularMid : []),
    ...(reachedLate ? popularLate : []),
  ];

  // Possession keys from the COMPLETE purchase log (incl. shards/consumables/components).
  // actualCore stays filtered for display; membership must not drop items like aghanims_shard.
  ...
  const actualCore = normalizedPurchases.filter((p) => isMajorPurchase(p.key, itemConstants));

  const actualKeys = new Set(normalizedPurchases.map((i) => i.key));

  // offMeta must use the purchase's own stage bucket — do not union sparse early data
  // onto mid/late purchases. Suppress when that stage's bucket is empty/unavailable.
  const offMeta = actualCore
    .filter((a) => {
      if (a.time <= 8 * 60) return false;
      const stage = a.time >= POPULARITY_LATE_START_SEC
        ? 'late'
        : (a.time >= POPULARITY_MID_START_SEC ? 'mid' : 'early');
      const stageKeys = stage === 'late' ? lateKeys : stage === 'mid' ? midKeys : earlyKeys;
      if (stageKeys.size === 0) return false;
      return !stageKeys.has(a.key);
    })
    .slice(0, 5)
    .map((a) => ({ key: a.key, name: a.name, timeLabel: a.timeLabel }));

  // Deduplicate by normalized key before UI slice (mid+late same item → one "Common missing").
  const seenMissingKeys = new Set();
  const missingPopular = missingCandidates
    .filter((p) => {
      if (!p.key || actualKeys.has(p.key) || seenMissingKeys.has(p.key)) return false;
      seenMissingKeys.add(p.key);
      return true;
    })
    .slice(0, 4)
...
export { isMajorPurchase, formatItemTime, CONSUMABLE_OR_COMPONENT, STANDALONE_MAJOR_PURCHASES, DOTABUFF_SLUG_BY_INTERNAL };
