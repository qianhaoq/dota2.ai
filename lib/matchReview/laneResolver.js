...
/** 至少 8 名选手有 lane_pos 才做三路聚类，避免少量样本误标 grounded */
const MIN_LANE_POS_PLAYERS = 8;
/** 每个簇至少 2 名选手，且三路质心 y 间距足够，才算有效分路 */
const MIN_CLUSTER_PLAYERS = 2;
const MIN_CENTROID_Y_GAP = 12;
...
/**
 * @typedef {object} ResolvedLane
 * @property {number} clusterId
 * @property {string} laneLabel
 * @property {{ x: number, y: number }} centroid
 * @property {number[]} radiantHeroIds
 * @property {number[]} direHeroIds
 * @property {string} source
 * @property {string} confidence
 */
/**
 * @typedef {object} ResolvedLanePlayer
 * @property {number} heroId
 * @property {number} playerSlot
 * @property {boolean} isRadiant
 * @property {string} laneLabel
 * @property {number} clusterId
 * @property {LaneCentroid | null} centroid
 * @property {Array<{ heroId: number }>} allies
 * @property {Array<{ heroId: number }>} opponents
 * @property {Array<{ heroId: number }>} nearby
 * @property {string} source
 * @property {string} confidence
...
/**
 * k-means 必须产出 3 个非空、足够分离的簇，否则不应标为 grounded。
 * @param {Array<{ x: number, y: number }>} centroids
 * @param {Record<string, Array<unknown>>} clusterMembers
 */
function clustersAreMeaningful(centroids, clusterMembers) {
  const ids = Object.keys(clusterMembers);
  if (ids.length !== 3 || centroids.length !== 3) return false;

  for (const id of ids) {
    if ((clusterMembers[id] || []).length < MIN_CLUSTER_PLAYERS) return false;
  }

  const ys = centroids.map((c) => c.y).sort((a, b) => a - b);
  if (ys[1] - ys[0] < MIN_CENTROID_Y_GAP || ys[2] - ys[1] < MIN_CENTROID_Y_GAP) {
    return false;
  }
 ...
 * @returns {{ lanes: ResolvedLane[], players: ResolvedLanePlayer[], source: string }}
 */
function ungroundedLaneResult(positions) {
  return {
    lanes: [],
...
export function resolveLanes(players) {
  ...
  if (withCentroid.length < MIN_LANE_POS_PLAYERS) {
    return ungroundedLaneResult(positions);
  }
  ...
  if (!clustersAreMeaningful(centroids, clusterMembers)) {
    return ungroundedLaneResult(positions);
  }
