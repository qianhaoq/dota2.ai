...
function buildEvidenceCatalog(matchFact, lang) {
  ...
  if (enr?.benchmarks?.lastHits) {
    catalog.push({
      ...
      label: isZh ? '补刀百分位' : 'LH percentile',
      value: `${enr.benchmarks.lastHits.actual} (~P${enr.benchmarks.lastHits.percentile})`,
    });
  }
  (enr?.itemCompare?.delayed || []).slice(0, 3).forEach((d, i) => {
    catalog.push({
      factKey: `item_delay_${i}`,
      label: isZh ? `出装偏晚·${d.name}` : `Delayed item·${d.name}`,
      value: isZh
        ? `${d.actualAt || '—'}（参考 ${d.expectedBy} 前）`
        : `${d.actualAt || '—'} (ref before ${d.expectedBy})`,
    });
  });
...
function evidenceSupportsCategory(category, evidence) {
  ...
  switch (category) {
    ...
    case 'farm_route':
      // Require benchmark / timing keys — plain gpm alone is not route evidence.
      return has((k) => (
        k === 'gpm_percentile'
        || k === 'xpm_percentile'
        || k === 'lh_percentile'
        || k === 'cs_at_10'
        || k.startsWith('item_delay_')
      ));
    default:
      return false;
  }
...
function isGroundedDrillStep(step, catalog, matchFact, lang) {
  ...
  return isSafeDrillTemplateStep(text, lang);
}
...
function buildFallbackDrill(matchFact, lang) {
  ...
  const lhPct = enr?.benchmarks?.lastHits?.percentile;
  const delayedItem = enr?.itemCompare?.delayed?.[0];

  const supportLean = assists >= 10 && assists >= kills;
  const highDeaths = deaths >= 6;
  const farmLean = gpm >= 520 && kills >= assists;
  const farmGap = (gpmPct != null && gpmPct <= 30) || (lhPct != null && lhPct <= 30);

  if (farmGap && !highDeaths) {
    const pctLabel = gpmPct != null ? `P${gpmPct}` : (lhPct != null ? `P${lhPct}` : '');
    return {
      duration: isZh ? '15 分钟' : '15 min',
      title: isZh ? '下一局：把经济拉回基准线' : 'Next game: lift farm to baseline',
      steps: isZh
        ? [
          `用 ${heroName} 打一局，10 分钟前只盯补刀与经验，目标超过本场百分位 ${pctLabel || '低位'}`,
          delayedItem
            ? `中期优先完成一件核心装（参考：本场 ${delayedItem.name} 偏晚）`
            : '中期先完成一件核心装，再考虑参团',
          '只练一件事：安全收线，不主动接没有视野的团',
        ]
        : [
          `Play one game as ${heroName}; before 10:00 focus CS/XP to beat this game's ${pctLabel || 'low'} percentile`,
          delayedItem
            ? `Finish one core item earlier (this game ${delayedItem.name} came late)`
            : 'Finish one core item before forcing fights',
          'One focus: take safe farm — skip foggy fights',
        ],
    };
  }
...
export function buildFallbackAiCards(matchFact, lang = 'zh', opts = {}) {
  ...
  let primary_mistake;
  if (farmGap) {
    const farmEvidence = resolveEvidence(
      [
        ...(catalog.some((e) => e.factKey === 'gpm_percentile') ? [{ factKey: 'gpm_percentile' }] : []),
        ...(catalog.some((e) => e.factKey === 'lh_percentile') ? [{ factKey: 'lh_percentile' }] : []),
        ...(catalog.some((e) => e.factKey === 'xpm_percentile') ? [{ factKey: 'xpm_percentile' }] : []),
        ...(catalog.some((e) => e.factKey === 'cs_at_10') ? [{ factKey: 'cs_at_10' }] : []),
        ...(delayedItem && catalog.some((e) => e.factKey === 'item_delay_0') ? [{ factKey: 'item_delay_0' }] : []),
      ].filter((e, i, arr) => arr.findIndex((x) => x.factKey === e.factKey) === i),
      catalog,
    );
    if (farmEvidence.length && evidenceSupportsCategory('farm_route', farmEvidence)) {
      const pctBits = [
        gpmPct != null ? `GPM P${gpmPct}` : null,
        lhPct != null ? (isZh ? `补刀 P${lhPct}` : `LH P${lhPct}`) : null,
      ].filter(Boolean).join(isZh ? '，' : ', ');
      primary_mistake = {
        category: 'farm_route',
        categoryLabel: MISTAKE_CATEGORIES.farm_route[isZh ? 'zh' : 'en'],
        headline: isZh
          ? (delayedItem
            ? `经济节奏偏慢，${delayedItem.name} 成型偏晚`
            : '经济/补刀低于同英雄基准')
          : (delayedItem
            ? `Farm tempo lagged — ${delayedItem.name} finished late`
            : 'Farm below hero benchmark'),
        explanation: isZh
          ? `MatchFact + OpenDota 基准：${lens?.displayName || '英雄'} KDA ${lens?.kda || '—'}，死亡 ${deaths}，GPM ${gpm}${pctBits ? `（${pctBits}）` : ''}。${delayedItem ? `${delayedItem.name} 于 ${delayedItem.actualAt} 成型。` : ''}只陈述差距，不做未证实因果。`
          : `MatchFact + OpenDota benchmarks: ${lens?.displayName || 'hero'} KDA ${lens?.kda || '—'}, ${deaths} deaths, GPM ${gpm}${pctBits ? ` (${pctBits})` : ''}. ${delayedItem ? `${delayedItem.name} at ${delayedItem.actualAt}. ` : ''}Stating gaps only; no unsupported causation.`,
  ...
  if (!primary_mistake) {
    ...
    if (canPrimaryMistake) {
      const deathHint = deaths >= 6
        ? (isZh ? `死亡 ${deaths} 次偏高，` : `${deaths} deaths is high; `)
        : '';
      primary_mistake = {
        category: 'fight_timing',
        categoryLabel: MISTAKE_CATEGORIES.fight_timing[isZh ? 'zh' : 'en'],
        headline: isZh
          ? (deaths >= 6 ? '高死亡局：按关键节点复盘团战节奏' : '按关键节点复盘团战与经济节奏')
          : (deaths >= 6 ? 'High deaths: review fight tempo at key nodes' : 'Review fight tempo and economy at key moments'),
        explanation: isZh
          ? (focus
            ? `${deathHint}仅陈述 MatchFact：${lens?.displayName || '英雄'} KDA ${lens?.kda || '—'}，死亡 ${deaths} 次，GPM ${gpm}${gpmPct != null ? `（约 P${gpmPct}）` : ''}。具体转折点见下方时间戳，不做未证实的因果推断。`
            : '按下方时间戳节点复盘团战与经济节奏，不做未证实的因果推断。')
          : (focus
            ? `${deathHint}MatchFact only: ${lens?.displayName || 'hero'} KDA ${lens?.kda || '—'}, ${deaths} deaths, GPM ${gpm}${gpmPct != null ? ` (~P${gpmPct})` : ''}. Turning points are listed below; no unsupported causal claims.`
