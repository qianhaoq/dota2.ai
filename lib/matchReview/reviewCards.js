...
const DRILL_ACTION_PRESCRIPTION_RE = /\b(cast|purchase|buy|use|activate|upgrade|stack|skill|spell|ability|ult|ultimate|itemize|farm|push|gank|roshan|ward|smoke)\b|放|买|施放|购买|升级/i;

/** Allow only reflective, role-neutral coaching phrases (fallback-drill style). */
const GENERIC_DRILL_COACHING_RE = /改进|信息|视野|人数|站位|撤退|专注|只练|disengage|vision|numbers|position|one focus|after each death|问自己|information|improvement|exit|reset|tempo|decision|within 10s/i;

/** Reject invented balance/patch/stat claims in mentor notes. */
const MENTOR_FACTUAL_CLAIM_RE = /\d+\s*%|patch\s*\d|版本|倍率|amplification|伤害|damage|cooldown|冷却|spell\s*amp|技能.*(加|增|倍)|balance|meta|削弱|增强|\bbuff\b|\bnerf\b/i;

/**
 * @param {Array} catalog
 * @param {object} matchFact
 */
function catalogAnchors(catalog, matchFact) {
  const anchors = [];
  for (const e of catalog) {
    if (e.label && e.label.length >= 2) anchors.push(e.label);
    if (e.value && String(e.value).length >= 2) anchors.push(String(e.value));
  }
  const hero = matchFact?.focusLens?.displayName;
  if (hero && hero.length >= 2) anchors.push(hero);
  return anchors;
}
 ...
 * @param {object} matchFact
 */
function stepReferencesCatalog(text, catalog, matchFact) {
  return catalogAnchors(catalog, matchFact).some((anchor) => text.includes(anchor));
}
 ...
 * @param {object} matchFact
 */
function isGroundedDrillStep(step, catalog, matchFact) {
  ...
  if (stepReferencesCatalog(text, catalog, matchFact)) return true;
  if (GENERIC_DRILL_COACHING_RE.test(text)) return true;
  return false;
}
 ...
 * @param {object} matchFact
 */
function areGroundedDrillSteps(steps, catalog, matchFact) {
  return Array.isArray(steps)
    && steps.length > 0
    && steps.every((s) => isGroundedDrillStep(s, catalog, matchFact));
}
 ...
 * @param {string} note
 */
function isGroundedMentorNote(note) {
  if (!note || typeof note !== 'string') return false;
  const text = note.trim();
  if (!text) return false;
  if (MENTOR_FACTUAL_CLAIM_RE.test(text)) return false;
  if (DRILL_ITEM_PRESCRIPTION_RE.test(text)) return false;
  return true;
}
...
function normalizeAiCards(raw, lang, catalog, matchDuration, matchFact) {
  ...
  const drill = raw.drill ? (() => {
    ...
    if (!areGroundedDrillSteps(steps, catalog, matchFact)) {
      return buildFallbackDrill(matchFact, lang);
    }
  ...
  const mentor_note = rawNote
    ? (isGroundedMentorNote(rawNote) ? rawNote : defaultMentorNote(lang))
    : undefined;
...
export { MISTAKE_CATEGORIES, PHASE_LABELS, localizeKillTarget, resolveEvidence, buildKeyMomentsFromTimeline, hasDistinctKeyMoments, isGroundedKeyMoment, isGroundedDrillStep, areGroundedDrillSteps, isGroundedMentorNote, defaultMentorNote, evidenceSupportsCategory, boundTimelineFactKey };
