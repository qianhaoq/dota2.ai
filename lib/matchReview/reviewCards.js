...
/** Exact safe mentor-note phrases (non-factual flavor only). */
const SAFE_MENTOR_NOTE_EXACT = new Set([
  '拉比克结语',
  'Rubick sign-off',
...
function isGroundedMentorNote(note, lang) {
  ...
  if (SAFE_MENTOR_NOTE_EXACT.has(text)) return true;
  return false;
}
...
export { MISTAKE_CATEGORIES, PHASE_LABELS, localizeKillTarget, resolveEvidence, buildKeyMomentsFromTimeline, hasDistinctKeyMoments, isGroundedKeyMoment, isGroundedDrillStep, areGroundedDrillSteps, isSafeDrillTemplateStep, isGroundedMentorNote, defaultMentorNote, evidenceSupportsCategory, boundTimelineFactKey };
