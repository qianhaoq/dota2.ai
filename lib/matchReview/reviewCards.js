...
const DRILL_ITEM_PRESCRIPTION_RE = /羊刀|sheep\s*stick|butterfly|bkb|black\s*king\s*bar|aghanim|紫苑|orchid|manta|diffusal|绝刃|nullifier|飓风|cyclone|force\s*staff|闪烁|blink|出.{0,6}(装|道具|item)|buy\s+[a-z]/i;
 ...
 * @param {Array} catalog
 */
function isGroundedDrillStep(step, catalog) {
 ...
 * @param {Array} catalog
 */
function areGroundedDrillSteps(steps, catalog) {
  return Array.isArray(steps)
    && steps.length > 0
    && steps.every((s) => isGroundedDrillStep(s, catalog));
}
...
function normalizeAiCards(raw, lang, catalog, matchDuration, matchFact) {
  ...
  const drill = raw.drill ? (() => {
    ...
    if (!areGroundedDrillSteps(steps, catalog)) {
...
function secondDefaultFollowup(lang, keyMoments = [], catalog = []) {
  ...
  return factBoundEconomyFollowup(lang, catalog);
}
...
function defaultFollowups(lang, keyMoments = [], catalog = []) {
  ...
  if (!hasTimelineMoment) {
    ...
    const secondChip = kda
      ? (lang === 'zh'
        ? `KDA ${kda.value} 反映了什么局面选择？`
        : `What did KDA ${kda.value} say about decision-making?`)
      : (lang === 'zh' ? '本局经济节点如何影响胜负？' : 'How did economy checkpoints affect the outcome?');
    return [economyChip, secondChip, practiceChip];
  }
  ...
  return [fightChip, economyChip, practiceChip];
}
...
export { MISTAKE_CATEGORIES, PHASE_LABELS, localizeKillTarget, resolveEvidence, buildKeyMomentsFromTimeline, hasDistinctKeyMoments, isGroundedKeyMoment, isGroundedDrillStep, areGroundedDrillSteps, evidenceSupportsCategory, boundTimelineFactKey };
