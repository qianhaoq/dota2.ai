...
function evidenceSupportsCategory(category, evidence) {
  ...
  switch (category) {
    ...
    case 'vision':
      return has((k) => k.startsWith('ward_') || k.startsWith('vision_'));
    case 'positioning':
      return has((k) => k === 'deaths' || k === 'kda');
    case 'farm_route':
...
function normalizeAiCards(raw, lang, catalog, matchDuration) {
  ...
  const followups = normalizeFollowups(raw.followups, lang, key_moments);
 ...
 */
function defaultFollowups(lang, keyMoments = []) {
  ...
  return lang === 'zh'
    ? [fightChip, '为什么不该出羊刀', '下一局只练一件事']
    : [fightChip, 'Why not Scythe?', 'One thing to practice next'];
}
 ...
 */
function normalizeFollowups(rawFollowups, lang, keyMoments) {
  const defaults = defaultFollowups(lang, keyMoments);
  ...
  if (!isBoundFightFollowup(mapped[0], lang, firstMoment)) {
    mapped[0] = defaults[0];
  }
  return mapped;
}
...
export function buildFallbackAiCards(matchFact, lang = 'zh', opts = {}) {
  ...
  const ai = {
    ...
    followups: includeFollowups ? defaultFollowups(lang, key_moments) : [],
    mentor_note: isZh
