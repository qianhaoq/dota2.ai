...
function hasDistinctKeyMoments(moments, minCount = 3) {
  ...
  for (const m of moments) {
    for (const e of m.evidence || []) {
      if (e?.factKey?.startsWith('timeline_')) timelineKeys.add(e.factKey);
    }
    if (typeof m.timestamp === 'number') timestamps.add(m.timestamp);
  }
  return timelineKeys.size >= minCount || timestamps.size >= minCount;
}
