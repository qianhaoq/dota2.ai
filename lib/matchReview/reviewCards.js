...
function evidenceSupportsCategory(category, evidence) {
  ...
  switch (category) {
    ...
    case 'positioning':
      return has((k) => k.startsWith('death_context_') || k.startsWith('position_'));
    case 'farm_route':
      return has((k) => k === 'cs_at_10' || k === 'gpm' || k.startsWith('gold_lead_'));
    default:
      return false;
  }
}
...
function normalizeAiCards(raw, lang, catalog, matchDuration) {
  ...
  const primary_mistake = raw.primary_mistake && VALID_CATEGORIES.has(rawCategory)
    ? (() => {
  ...
  const key_moments = Array.isArray(raw.key_moments)
    ? raw.key_moments
      .map((m) => {
        ...
        const headline = nonEmptyString(m.headline);
        const why = nonEmptyString(m.why);
 ...
 * @param {unknown} chip
 * @param {string} lang
 * @param {{ timestampLabel?: string; timestamp?: number; headline?: string } | null} firstMoment
 */
function isBoundFightFollowup(chip, lang, firstMoment) {
  if (!firstMoment || typeof chip !== 'string') return false;
  const ts = firstMoment.timestampLabel
    || (firstMoment.timestamp != null ? formatTimestamp(firstMoment.timestamp) : null);
  const headline = firstMoment.headline?.trim();
  if (!ts || !headline) return false;
  const trimmed = chip.trim();
  if (!trimmed) return false;
  if (lang === 'zh' && (trimmed === '展开这场团' || trimmed === '展开关键团战节点')) return false;
  if (lang !== 'zh' && (/^break down that fight$/i.test(trimmed) || /^break down the key fight$/i.test(trimmed))) {
    return false;
  }
  return trimmed.includes(ts) && trimmed.includes(headline);
}
...
function normalizeFollowups(rawFollowups, lang, keyMoments, catalog = []) {
  ...
  const firstMoment = Array.isArray(keyMoments) && keyMoments.length > 0 ? keyMoments[0] : null;
  for (let i = 0; i < 3; i++) {
    const chip = mapped[i];
    const fallback = defaults[i];
    if (!chip || !fallback) {
      if (fallback) mapped[i] = fallback;
      continue;
    }
    let supported = true;
    if (i === 0) {
      supported = isBoundFightFollowup(chip, lang, firstMoment);
    } else if (isItemSpecificFollowup(chip, lang)) {
      supported = false;
    } else if (i === 1) {
      supported = isEvidenceBoundSecondaryFollowup(chip, lang, keyMoments, catalog, fallback);
    } else if (i === 2) {
      supported = chip === fallback;
    }
    if (!supported) mapped[i] = fallback;
  }
 ...
 */
function isEvidenceBoundSecondaryFollowup(chip, lang, keyMoments, catalog, fallback) {
  if (chip === fallback) return true;
  const second = Array.isArray(keyMoments) && keyMoments.length > 1 ? keyMoments[1] : null;
  if (second) {
    const ts = second.timestampLabel
      || (second.timestamp != null ? formatTimestamp(second.timestamp) : null);
    const headline = second.headline?.trim();
    if (ts && headline && chip.includes(ts) && chip.includes(headline)) return true;
  }
  for (const entry of catalog) {
    if (
      (entry.factKey.startsWith('gold_lead_') || entry.factKey === 'gpm')
      && chip.includes(entry.label)
      && chip.includes(entry.value)
    ) {
      return true;
    }
  }
 ...
 * @param {string} lang
 */
function isItemSpecificFollowup(chip, lang) {
  if (typeof chip !== 'string') return false;
  const trimmed = chip.trim();
  if (!trimmed) return false;
  if (lang === 'zh') {
    return /羊刀|出装|装备选择|蝴蝶|辉耀|大炮|散夜对剑/.test(trimmed);
  }
  return /scythe|item build|itemisation|itemization|butterfly|manta|blink|desolator/i.test(trimmed);
}
