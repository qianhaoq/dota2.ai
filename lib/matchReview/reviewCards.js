...
export function isReviewAiCardsComplete(reviewCards) {
  ...
  const momentsGrounded = moments.length >= 3
    && moments.every((m) => Array.isArray(m.evidence) && m.evidence.length >= 1);
  return Boolean(
...
/**
 * Build key moments from one timeline selection so headline and evidence match.
 * @param {object} matchFact
 * @param {string} lang
 * @param {number} [limit=5]
 */
function buildKeyMomentsFromTimeline(matchFact, lang, limit = 5) {
  const isZh = lang === 'zh';
  const timeline = selectTimelineForDisplay(matchFact.timeline || [], limit);
  return timeline.slice(0, 4).map((ev, idx) => {
    const factKey = `timeline_${idx}`;
    const label = formatTimestamp(ev.time);
    const value = formatObjectiveLabel(ev, lang);
...
export function buildFallbackAiCards(matchFact, lang = 'zh', opts = {}) {
  ...
  const key_moments = buildKeyMomentsFromTimeline(matchFact, lang, 5);
  ...
  const ai = {
    ...
    followups: includeFollowups ? defaultFollowups(lang) : [],
    mentor_note: isZh
...
export { MISTAKE_CATEGORIES, PHASE_LABELS, localizeKillTarget, resolveEvidence, buildKeyMomentsFromTimeline };
