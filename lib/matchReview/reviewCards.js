 ...
 * @param {Array} catalog
 */
function bindKeyMomentTimestamp(m, catalog) {
...
function hasTimelineEvidence(evidence) {
  return Array.isArray(evidence)
    && evidence.some((e) => e?.factKey?.startsWith('timeline_'));
}
...
function evidenceSupportsCategory(category, evidence) {
  ...
  switch (category) {
    ...
    case 'positioning':
      return has((k) => k === 'deaths' || k === 'kda' || k.startsWith('timeline_'));
    case 'farm_route':
...
function boundTimelineFactKey(moment) {
  return (moment?.evidence || [])
    .map((e) => e?.factKey)
    .find((key) => key && key.startsWith('timeline_')) || null;
}
...
function normalizeAiCards(raw, lang, catalog, matchDuration) {
  ...
  const primary_mistake = raw.primary_mistake && VALID_CATEGORIES.has(rawCategory)
    ? (() => {
      const headline = String(raw.primary_mistake.headline || '').trim();
      const explanation = String(raw.primary_mistake.explanation || '').trim();
  ...
  const key_moments = Array.isArray(raw.key_moments)
    ? raw.key_moments
      .map((m) => {
        ...
        const timestamp = bindKeyMomentTimestamp({ evidence: m.evidence }, catalog);
