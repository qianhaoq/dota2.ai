 ...
 * @param {string} text
 */
function isGroundedDrillMetadata(text) {
  const value = typeof text === 'string' ? text.trim() : '';
  if (!value) return false;
  if (DRILL_ITEM_PRESCRIPTION_RE.test(value)) return false;
  if (DRILL_LANE_FARM_RE.test(value)) return false;
  if (DRILL_ACTION_PRESCRIPTION_RE.test(value)) return false;
  return true;
}
...
function normalizeAiCards(raw, lang, catalog, matchDuration, matchFact) {
  ...
  const drill = raw.drill ? (() => {
    ...
    if (!isGroundedDrillMetadata(title) || !isGroundedDrillMetadata(duration)) {
      return {
