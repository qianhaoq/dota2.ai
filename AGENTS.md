...
- Any draft / side / practice change that invalidates in-flight Analyze / Playbook / Next-pick MUST bump `contextRevision` **and** call `cancelStream()` together, and retire stale suggestion cards (`clearStaleSuggestionsForRevision`).
...
- DeepSeek output language follows UI `lang` (zh → 简体中文, en → English); do not flip the default-language product decision (中文 first).
